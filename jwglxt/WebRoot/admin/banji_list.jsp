...
<table width="920" border="0" cellspacing="0" cellpadding="0" background="<%=path%>/img/all_bg.jpg">
  <tr>
    <td class="border_left border_right border_bottom" height="400" valign="top" align="center"><table border="0" cellspacing="0" cellpadding="0">
      ...
      <form name="form1" method="post" action="<%=path %>/admin/banji_list.jsp">
      ...
      <table width="98%" border="0" cellspacing="0" cellpadding="0" bgcolor="#999999">
        <tr class="tabletop">
          <td height="25" colspan="12" align="center">班级管理</td>
        </tr>
        <tr class="tablemenu">
        ...
        <tr class="tabletd<%=i%2+1 %>">
          ...
          <td height="25" align="center"><a title="点击查看详细信息" href="<%=path %>/admin/banji_show.jsp?id=<%=alRow.get(0) %>"><%=alRow.get(1) %></a></td>
          <td height="25" align="center"><%=alRow.get(2) %></td>
          <td height="25" align="center"><%=alRow.get(3).equals("0")?"未指定班主任":"<a title='点击查看详细信息' href='"+path+"/admin/laoshi_show.jsp?id="+laoshi.get(0)+"'>"+laoshi.get(4)+"</a>" %></td>
          <td height="25" align="center"><%=alRow.get(4) %></td>
          <td height="25" align="center"><%=alRow.get(5) %></td>
          <td height="25" align="center"><a href="<%=path %>/admin/banji.jsp?mark=update&id=<%=alRow.get(0) %>">修改</a>&nbsp;&nbsp;<a href="<%=path %>/DelServlet?banji=<%=alRow.get(0) %>">删除</a></td>
        </tr>
        <%} %>
        <tr class="tabletop">
          <td height="25" colspan="12" align="center"><a href="<%=path %>/admin/banji.jsp?mark=add">添加班级</a></td>
        </tr>
      </table></td>
  </tr>
</table>
